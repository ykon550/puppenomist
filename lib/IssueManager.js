...
class IssueManager {
    ...
    async selectTarget(page) {
        switch(this.mode){
            case MODE.RESUME:
                this._issueLinks = this.status.issues.filter((issue) => {issue.status == STATUS.LINK_EXTRACTED});
            case MODE.FRESH:
    ...
    setStarted(url){
        const idx = getIndex(this.status, "url", url);
        this.status[idx].status = STATUS.SCRAPE_STARTED;       
        console.log("started " + url);
    }

    setDone(url){
        const idx = getIndex(this.status, "url", url);
        this.status[idx].status = STATUS.DONE;
